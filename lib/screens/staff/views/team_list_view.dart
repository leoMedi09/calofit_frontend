...
class _TeamListViewState extends State<TeamListView> {
  ...
  Future<void> _loadTeam() async {
    ...
    try {
      final List<User> team =
          await _apiService.getUsers(authProvider.token ?? '');
  ...
  void _applyFilters() {
    setState(() {
      _filteredMembers = _allMembers.where((member) {
        final matchesSearch = member.fullName
                .toLowerCase()
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          ...
          _buildFilterChip(
              'Nutricionistas', 'nutritionist', Icons.restaurant_menu_rounded),
          const SizedBox(width: 10),
          _buildFilterChip(
              'Administradores', 'admin', Icons.admin_panel_settings_rounded),
        ],
      ),
    );
  }
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          if (member.pacientesCount != null &&
                              member.pacientesCount! > 0) ...[
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              const Text(
                  'Establece las nuevas credenciales para este miembro del equipo.'),
                  ...
                  : () async {
                      ...
                      try {
                        ...
                        await _apiService.updateStaffPassword(
                            member.id, password, authProvider.token ?? '');
                        if (context.mounted) {
  ...
  void _showEditStaffDialog(BuildContext context, User member) {
    ...
    final TextEditingController paternalController =
        TextEditingController(text: member.lastNamePaternal);
    final TextEditingController maternalController =
        TextEditingController(text: member.lastNameMaternal);
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                _buildEditField(
                    firstNameController, 'Nombre', Icons.person_outline),
                const SizedBox(height: 12),
                _buildEditField(paternalController, 'Apellido Paterno',
                    Icons.person_outline),
                const SizedBox(height: 12),
                _buildEditField(maternalController, 'Apellido Materno',
                    Icons.person_outline),
                const SizedBox(height: 12),
                _buildEditField(emailController, 'Correo Corporativo',
                    Icons.email_outlined),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  value: ['nutritionist', 'admin']
                  ...
                  : () async {
                      ...
                      try {
                        ...
                        await _apiService.updateStaff(
                            member.id, updateData, authProvider.token ?? '');
              ...
              onPressed: isLoading
                  ...
                  : () async {
                      ...
                      try {
                        final authProvider =
                            Provider.of<AuthProvider>(context, listen: false);
                        await _apiService.updateStaffStatus(
                            member.id, authProvider.token ?? '');
